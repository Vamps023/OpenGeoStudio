...
// ─── Shared numerical integration (Simpson's rule) ───────────
//
// Used by clothoid.hpp (Fresnel integrals) and geometry_segment.hpp
// (SpiralSegment::evaluateDS). Previously duplicated in both files.
//
// Integrates f(t) from 0 to upperLimit using Simpson's rule with n
// subdivisions (n must be even; will be adjusted if odd).
//
template<typename Func>
inline std::pair<double, double> simpsonIntegrate2D(
    Func f, double upperLimit, int n = 100
) {
    if (n % 2 != 0) n++;  // Simpson's rule requires even n
    if (n < 2) n = 2;
    double dt = upperLimit / n;
    double x = 0, y = 0;
    for (int i = 0; i <= n; i++) {
...
// ─── Shared circle arc sampling ──────────────────────────────
//
// Samples points on a circular arc defined by center, radius, start
// angle, and sweep angle. Used by arc.hpp (computeCircleArc,
// filletArc) and skia_arc.hpp (computeSkiaArcPath).
//
inline std::vector<Point2D> sampleCircleArc(
