...
namespace LM
{
    void TouchController::Update(QList<QEventPoint> points, Camera& camera)
    {
        ...
        if (points.size() >= 1)
            p1 = QVector2D(points[0].position());
        if (points.size() >= 2)
            p2 = QVector2D(points[1].position());
        ...
        else if (touchMode == TouchMode::DualTouch)
        {
            ...
            camera.translate((lastGroundPos - currGroundPos).toVector3D());
            lastHandlePos = pm;
        }
    }
    ...
    void FreeRotController::Update(QVector2D p, Camera& camera)
    {
        ...
        auto rotated = backupRotation.inverted() * camera.rotation();
