...
inline CircleArc computeCircleArc(const Point2D& startPoint,
    ...
    for (int i = 0; i <= segments; i++) {
        double t = static_cast<double>(i) / segments;
        double angle = arc.startAngle + arc.sweep * t;
        arc.points.push_back({arc.center.x + radius * std::cos(angle),
                              arc.center.y + radius * std::sin(angle)});
    }
...
inline std::vector<Point2D> filletArc(const Point2D& corner,
    ...
    // Sample circular arc from p1 to p2 around center
