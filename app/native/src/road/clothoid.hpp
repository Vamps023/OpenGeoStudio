...
// S(t) = ∫₀ᵗ sin(πu²/2) du
//
// We use numerical integration (Simpson's rule) for accuracy.
...
inline FresnelResult fresnel(double t) {
    // Use Simpson's rule with adaptive subdivisions
    const int n = 1000;  // number of subdivisions
    const double dt = t / n;
    double c = 0, s = 0;
    ...
    for (int i = 0; i <= n; i++) {
        ...
        // Simpson's rule weight: 1, 4, 2, 4, 2, ..., 4, 1
        double weight = 1.0;
        if (i > 0 && i < n) {
...
inline ClothoidResult computeClothoid(const ClothoidParams& params, int segments = 64) {
    ...
    for (int i = 0; i <= segments; i++) {
        ...
        //
        // We use direct numerical integration (Simpson's rule)
        const int subN = 50;
        double ds = s / subN;
        double x = 0, y = 0;
